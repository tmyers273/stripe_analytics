...
export function NewBizReactivationChart({
  data,
}: NewBizReactivationChartProps) {
    ...
    const initializeChart = () => {
      ...
      if (rect.width === 0 || rect.height === 0) {
        ...
        if (retryCount <= maxRetries) {
          console.warn(
            `NewBizReactivation chart container has no dimensions, retry ${retryCount}/${maxRetries}...`
          );
          setTimeout(initializeChart, retryDelay);
          return;
        } else {
          console.error(
            "NewBizReactivation chart container still has no dimensions after max retries"
          );
          return;
        }
      }
        ...
        tooltip: {
          ...
          formatter: (params: any) => {
            ...
            const previousQuarterData =
              currentIndex > 0 ? quarterData[currentIndex - 1] : null;
            ...
            params.forEach((param: any) => {
              if (param.value !== 0) {
                ...
                const formattedValue =
                  valueInDollars > 0
                    ? `+$${valueInDollars.toLocaleString()}`
                    : `-$${Math.abs(valueInDollars).toLocaleString()}`;
                ...
                if (previousQuarterData) {
                  ...
                  if (percentChange !== 0) {
                    ...
                    if (Math.abs(percentChange) >= 999999) {
                      ...
                      const changeColor =
                        percentChange > 0 ? "#10b981" : "#ef4444";
                      displayText = `<span style="color: ${changeColor}; font-size: 11px;"> (${changeSign}${percentChange.toFixed(
                        1
                      )}%)</span>`;
                    }
            ...
            if (previousQuarterData) {
              ...
              if (netPercentChange !== 0) {
                ...
                if (Math.abs(netPercentChange) >= 999999) {
                  ...
                  const changeColor =
                    netPercentChange > 0 ? "#10b981" : "#ef4444";
                  displayText = `<span style="color: ${changeColor}; font-size: 11px;"> (${changeSign}${netPercentChange.toFixed(
                    1
                  )}%)</span>`;
                }
            ...
            Net: ${
              totalInDollars > 0 ? "+" : ""
            }$${totalInDollars.toLocaleString()}${netChangeText}
          </div>`;
